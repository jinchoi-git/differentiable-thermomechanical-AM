...
def main_function(params):
    raw = params[:power_on_steps]
    control_soft = 0.75 + 0.5 * jax.nn.sigmoid(raw)
    control = jnp.concatenate((control_soft, jnp.zeros((steps - power_on_steps,))), axis=0)

    temperatures = jax.jit(simulate_temperature)(control)
    # Pick a tiny “ROI” (e.g., top surface nodes) and a late time so thermal dynamics had time to react
    roi = (nodes[:,2] > -2.0).astype(jnp.float64)  # tweak if you like
    loss = jnp.mean(temperatures[-1] * roi)        # purely thermal loss
    return loss, control

# 1) Loss that only depends on temperatures (no control here)
def mechanics_loss_from_T(temperatures):
    # mechanics takes the full temperature sequence
    S_seq = jax.jit(simulate_mechanics)(temperatures)  # your existing function
    S_final = S_seq[-1]                                # last time
    Sxx, Syy, Szz, Sxy, Syz, Sxz = (S_final[:,0], S_final[:,1], S_final[:,2],
...
# 2) Tiny finite-difference on a single temp entry
def fd_on_T(temperatures, t_idx, node_idx, eps=1e-5):
    T0 = temperatures
    def bump(val):
        T = T0.at[t_idx, node_idx].set(T0[t_idx, node_idx] + val)
        return float(mechanics_loss_from_T(T))
    return (bump(eps) - bump(-eps)) / (2*eps)

def mech_grad_probe(temperatures, picks=3):
    # choose a few (t,node) points (e.g., late times & top surface nodes)
    T0 = temperatures
    dL_dT = jax.grad(mechanics_loss_from_T)(T0)
    dL_dT = np.array(dL_dT)

    # simple picks: last 3 times, first node (adjust as you wish)
    times = [T0.shape[0]-k for k in range(1, picks+1)]
    nodes = [0]*picks

    rows = []
    for (ti, ni) in zip(times, nodes):
        ad = dL_dT[ti, ni]
        fd = fd_on_T(T0, ti, ni, eps=1e-5)
        rows.append((ti, ni, float(ad), float(fd), abs(ad-fd)))
    return rows

raw = params_init[:power_on_steps]
control_soft = 0.75 + 0.5 * jax.nn.sigmoid(raw)
control = jnp.concatenate((control_soft, jnp.zeros((steps - power_on_steps,))), axis=0)
temperatures = jax.jit(simulate_temperature)(control)

rows = mech_grad_probe(temperatures, picks=3)
for (ti, ni, ad, fd, err) in rows:
    print(f"(t={ti}, node={ni})  AD={ad:.6e}  FD={fd:.6e}  |err|={err:.2e}")
