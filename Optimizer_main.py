...
def grad_check(params, eps=1e-3, n_checks=5):
    ...
    header = f"{'idx':>3} | {'autodiff':>12} | {'numeric':>12} | {'abs err':>10} | {'rel err':>10}"
    print(header)
    print("-" * len(header))
...
'''
# ===== Robust gradcheck utils (drop-in) =====
import jax
import jax.numpy as jnp
# Enable float64 for stable finite differences
jax.config.update("jax_enable_x64", True)
def _as_f64(x):
    x = jnp.asarray(x)
    return x.astype(jnp.float64) if x.dtype != jnp.float64 else x
def _adaptive_eps(x, rel=1e-6, abs_min=1e-12):
    # Per-parameter central-diff step: scales with magnitude, never below abs_min
    x = _as_f64(x)
    return jnp.maximum(abs_min, rel * (1.0 + jnp.abs(x)))
def _numeric_grad_central(f, x, eps_vec):
    """
    Central-difference numeric gradient of scalar f at x.
    x: 1-D array (vector of params).
    eps_vec: same shape as x with per-dimension steps.
    """
    x = _as_f64(x)
    eps_vec = _as_f64(eps_vec)
    n = x.size
    def fd_i(i):
        ei = jnp.zeros_like(x).at[i].set(1.0)
        e = eps_vec[i]
        return (f(x + e * ei) - f(x - e * ei)) / (2.0 * e)
    # vmap over indices for speed and JIT-friendliness
    g_fd = jax.vmap(fd_i)(jnp.arange(n))
    return g_fd
def check_grad_table(loss_fn, x, *, rel_step=1e-6, abs_step_min=1e-12, jit=True, print_table=True):
    """
    Drop-in replacement. Returns:
      {
        'value': float,
        'autodiff': jnp.ndarray,
        'numeric': jnp.ndarray,
        'abs_err': jnp.ndarray,
        'rel_err': jnp.ndarray,
      }
    """
    x = _as_f64(x)
    f = jax.jit(loss_fn) if jit else loss_fn
    # AD gradient at x
    val, g_ad = jax.value_and_grad(f)(x)
    g_ad = _as_f64(g_ad)
    # Numeric gradient (central diff with adaptive step)
    eps_vec = _adaptive_eps(x, rel=rel_step, abs_min=abs_step_min)
    g_fd = _numeric_grad_central(f, x, eps_vec)
    abs_err = jnp.abs(g_ad - g_fd)
    rel_err = abs_err / jnp.maximum(jnp.abs(g_fd), 1e-12)
    if print_table:
        # Pretty print (matches your earlier layout)
        header = "idx |     autodiff |      numeric |    abs err |    rel err"
        sep    = "-----+--------------+--------------+------------+------------"
        print(header)
        print(sep)
        for i in range(x.size):
            print(f"{i:3d} | {g_ad[i]: .6e} | {g_fd[i]: .6e} | {abs_err[i]: .2e} | {rel_err[i]: .2e}")
    return {
        "value": float(val),
        "autodiff": g_ad,
        "numeric": g_fd,
...
def check_direction(loss_fn, x, *, key=0, rel_step=1e-6, abs_step_min=1e-12, jit=True, print_line=True):
    """
    Directional derivative check: compares <gradf(x), v> vs central FD along v.
    Returns (ad, fd, abs_err, rel_err).
    """
    x = _as_f64(x)
    f = jax.jit(loss_fn) if jit else loss_fn
    # random unit direction
    k = jax.random.PRNGKey(key)
    v = jax.random.normal(k, x.shape)
    v = v / (jnp.linalg.norm(v) + 1e-12)
    # AD directional derivative
    ad_dir = jnp.vdot(jax.grad(f)(x), v)
    # FD directional derivative with a single scalar step
    # (scale with ||x|| so it's not too tiny/huge)
    base = 1.0 + jnp.linalg.norm(x) / (1.0 + x.size)
    eps = jnp.maximum(abs_step_min, rel_step * base)
    fd_dir = (f(x + eps * v) - f(x - eps * v)) / (2.0 * eps)
    abs_err = jnp.abs(ad_dir - fd_dir)
    rel_err = abs_err / jnp.maximum(jnp.abs(fd_dir), 1e-12)
    if print_line:
        print(f"directional  <gradf, v>: AD={ad_dir:.6e}  FD={fd_dir:.6e}  abs={abs_err:.2e}  rel={rel_err:.2e}")
    return float(ad_dir), float(fd_dir), float(abs_err), float(rel_err)
# Keep your loss_fn(control) the same. Just call:
def loss_fn(p):
    return main_function(jnp.array(p))[0]
control0 = jnp.ones((N_BLOCKS,))  # example input
g_line = check_direction(loss_fn, control0)     # quick scalar check
res    = check_grad_table(loss_fn, control0)    # full per-index table
'''
...
if __name__ == "__main__":
    ...
    if mode == "gradcheck":
        ...
        try:
            ctrl_path, src_run_dir = latest_control_under(work_dir, "bfgs")
        except FileNotFoundError as e:
