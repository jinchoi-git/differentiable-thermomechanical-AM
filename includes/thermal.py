...
def calc_mass_stiff(element, cp, cond, temperature, nodes, Bip_ele, Nip_ele, density):
    ...
    mass = (density * cp * detJac)[:, jnp.newaxis] * jnp.sum(
        Nip_ele[:, :, jnp.newaxis] @ Nip_ele[:, jnp.newaxis, :], axis=0
    )
    lump_mass = jnp.sum(mass, axis=1)
    stiffness = jnp.sum(
        (cond * detJac)[:, jnp.newaxis] * jnp.matmul(gradN_T, gradN), axis=0
    )
    stiff_temp = jnp.matmul(stiffness, temperature_nodes)
    return lump_mass, stiff_temp


def update_mvec_stiffness(
    m_vec,
    rhs,
    cps,
    conds,
    ...
    lump_mass, stiff_temp = jax.vmap(
        calc_mass_stiff, in_axes=(0, 0, 0, None, None, None, None, None)
    )(elements, cps, conds, temperature, nodes, Bip_ele, Nip_ele, density)
...
def calc_flux(
    ...
    r2 = jnp.square(
        jnp.linalg.norm(ip_pos - laser_loc[jnp.newaxis, t].repeat(4, axis=0), axis=1)
    )
    qmov = (
        2.0
        * base_power
        * laser_ont
        * control
        / (3.14 * jnp.square(r_beam))
        * jnp.exp(-2.0 * r2 / (jnp.square(r_beam)))
        * surface_xy
    )
...
def simulate_temperature(control, tctx):
    ...
    def thermal_step(carry, t_control):
        ...
        current_time = t * dt
        ae, an, asurf = update_birth(
            current_time, element_birth, node_birth, surface_birth
        )
        ...
        m_vec = jnp.zeros(n_n)
        rhs = jnp.zeros(n_n)
        m_vec, rhs = update_mvec_stiffness(
            m_vec,
            rhs,
            cps,
            conds,
