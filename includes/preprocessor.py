        ...
        while True:
            ...
            if line.split()[0] == "*ELEMENT_SOLID":
                ...
                while True:
                    ...
                    elements.append(
                        [
                            int(text[2]) - node_base,
                            int(text[3]) - node_base,
                            int(text[4]) - node_base,
                            int(text[5]) - node_base,
                            int(text[6]) - node_base,
                            int(text[7]) - node_base,
                            int(text[8]) - node_base,
                            int(text[9]) - node_base,
                        ]
                    )
            if line.split()[0] == "*END":
                break
...
def assign_birth_time(
    ...
    for i in range(1, toolpath.shape[0]):
        ...
        X = np.interp(
            t,
            [toolpath[i - 1, 0], toolpath[i, 0]],
            [toolpath[i - 1, 1], toolpath[i, 1]],
        )
        Y = np.interp(
            t,
            [toolpath[i - 1, 0], toolpath[i, 0]],
            ...
            for j in range(num):
                for k in range(ele_nodes.shape[0]):
                    if element_birth[k] == -1 and ele_topz[k] <= toolpath[i, 3] + 1e-5:
                        ...
                        distance1 = abs(
                            (ele_ctrl[k, 0] - X[j]) * dir_norm[0]
                            + (ele_ctrl[k, 1] - Y[j]) * dir_norm[1]
                        )
                        distance2 = distance - distance1
                        if (
                            distance1 < radius**2 + 1e-5
                            and distance2 < radius**2 + 1e-5
                        ):
                            element_birth[k] = t[j]
        # no birth
        if mode == 2:
...
def write_birth(
    ...
    element_birth = np.concatenate(
        (np.arange(0, elements.shape[0])[:, np.newaxis], element_birth[:, np.newaxis]),
        axis=1,
    )
    ...
    for i in range(elements.shape[0]):
        ...
        for j in range(6):
            ...
            if birth_neighbor > birth_current:
                ...
                if (
                    abs(nodes[element[index[j]]][0, 2] - nodes[element[index[j]]][1, 2])
                    < 1e-2
                    and abs(
                        nodes[element[index[j]]][1, 2] - nodes[element[index[j]]][2, 2]
                    )
                    < 1e-2
                ):
...
def get_toolpath(toolpath_raw, dt, endtime):
    ...
    laser_state = laser_state * (
        time <= toolpath_raw[-1, 0]
    )  # if time >= toolpath time, stop laser
...
def derivate_shape_fnc_element(parCoord):
    ...
    B = 0.1250 * np.array(
        [
            [
                -oneMinusEta * oneMinusZeta,
                oneMinusEta * oneMinusZeta,
                onePlusEta * oneMinusZeta,
                -onePlusEta * oneMinusZeta,
                -oneMinusEta * onePlusZeta,
                oneMinusEta * onePlusZeta,
                onePlusEta * onePlusZeta,
                -onePlusEta * onePlusZeta,
            ],
            [
                -oneMinusChsi * oneMinusZeta,
                -onePlusChsi * oneMinusZeta,
                onePlusChsi * oneMinusZeta,
                oneMinusChsi * oneMinusZeta,
                -oneMinusChsi * onePlusZeta,
                -onePlusChsi * onePlusZeta,
                onePlusChsi * onePlusZeta,
                oneMinusChsi * onePlusZeta,
            ],
            [
                -oneMinusChsi * oneMinusEta,
                -onePlusChsi * oneMinusEta,
                -onePlusChsi * onePlusEta,
                -oneMinusChsi * onePlusEta,
                oneMinusChsi * oneMinusEta,
                onePlusChsi * oneMinusEta,
                onePlusChsi * onePlusEta,
                oneMinusChsi * onePlusEta,
...
def derivate_shape_fnc_surface(parCoord):
    ...
    B = 0.25 * np.array(
        [
            [-oneMinusEta, oneMinusEta, onePlusEta, -onePlusEta],
...
class domain_mgr:
    def __init__(self, filename, sort_birth=True):
        ...
        self.Nip_ele = cp.array(
            [shape_fnc_element(parCoord) for parCoord in parCoords_element]
        )[:, :, np.newaxis]
        self.Nip_ele = cp.array(
            [shape_fnc_element(parCoord) for parCoord in parCoords_element]
        )
        self.Bip_ele = cp.array(
            [derivate_shape_fnc_element(parCoord) for parCoord in parCoords_element]
        )
        self.Nip_sur = cp.array(
            [shape_fnc_surface(parCoord) for parCoord in parCoords_surface]
        )
        self.Bip_sur = cp.array(
            [derivate_shape_fnc_surface(parCoord) for parCoord in parCoords_surface]
        )
                ...
                # option **ELEMENT_SOLID
                    ...
                    while True:
                        ...
                        if line[0] == "*":
                            break
                        if (
                            line.split()[1] == "Radiation"
                            or line.split()[1] == "radiation"
                        ):
                            line = next(f)
                            self.h_rad = float(line.split()[2])
                        if (
                            line.split()[1] == "convection"
                            or line.split()[1] == "Convection"
                        ):
                            line = next(f)
                ...
                if line.split()[0] == "*DEFINE_CURVE":
                    ...
                    while True:
                        ...
                        if line[0] == "$":
                            continue
                        text = line.split()
                        birth_list_element[int(float(text[1])) - element_base] = float(
                            text[0]
                        )
                if line.split()[0] == "*END":
                    break
    ...
    def generate_surf(self):
        ...
        conn_to_el_Vec = (
            np.arange(0, ele_num)[:, np.newaxis].repeat(8, axis=1).flatten()
        )
    ...
    def update_birth(self):
        ...
        self.active_surface = (self.surface_birth[:, 0] <= self.current_time) * (
            self.surface_birth[:, 1] > self.current_time
        )

    def get_ele_J(self):
        nodes_pos = self.nodes[self.elements]
        Jac = cp.matmul(
            self.Bip_ele, nodes_pos[:, cp.newaxis, :, :].repeat(8, axis=1)
    ...
    def get_surf_ip_pos_and_J(self):
        ...
        Jac = cp.matmul(
            self.Bip_sur, mapped_surf_nodes_pos[:, cp.newaxis, :, :].repeat(4, axis=1)
        )
    ...
    def get_timestep(self):
        ...
        Jac = cp.matmul(
            self.Bip_sur,
            mapped_surf_nodes_pos[:, :, cp.newaxis, :, :].repeat(4, axis=2),
        )
...
class heat_solve_mgr:
    ...
    def update_cp_cond(self):
        ...
        temperature_ip = (
            domain.Nip_ele[:, cp.newaxis, :]
            @ temperature_nodes[:, cp.newaxis, :, cp.newaxis].repeat(8, axis=1)
        )[:, :, 0, 0]
        ...
        for i in range(len(domain.mat_thermal)):
            ...
            self.density_Cp_Ip[domain.active_elements * mat] += (
                density * latent * (thetaIp > solidus) * (thetaIp < liquidus)
            )

            if domain.mat_thermal[i][5] == -1:
                temp_Cp = cp.asarray(domain.thermal_TD[matID][0][:, 0])
                Cp = cp.asarray(domain.thermal_TD[matID][0][:, 1])
                self.density_Cp_Ip[domain.active_elements * mat] += density * cp.interp(
                    thetaIp, temp_Cp, Cp
                )
            else:
            ...
            if domain.mat_thermal[i][6] == -1:
                ...
                self.Cond_Ip[domain.active_elements * mat] += cp.interp(
                    thetaIp, temp_Cond, Cond
                )
            else:
    ...
    def update_mvec_stifness(self):
        ...
        mass = cp.sum(
            (density_Cp_Ip * detJac)[:, :, cp.newaxis, cp.newaxis]
            * Nip_ele[:, :, cp.newaxis]
            @ Nip_ele[:, cp.newaxis, :],
            axis=1,
        )
        ...
        stiffness = cp.sum(
            (Cond_Ip * detJac)[:, :, cp.newaxis, cp.newaxis]
    ...
    def update_fluxes(self):
        ...
        q = ((qmov + qrad + qconv) * detJac)[:, :, cp.newaxis].repeat(
            4, axis=2
        ) * Nip_sur
    ...
    def time_integration(self):
        ...
        self.temperature[domain.active_nodes] += (
            domain.dt * self.rhs[domain.active_nodes] / self.m_vec[domain.active_nodes]
        )
        # modification required
        if self.isothermal == 1:
            self.temperature[cp.where(domain.nodes[:, 2] == -self.height)] = (
                self.ambient
            )
    ...
    def calculate_melt(self, solidus):
        ...
        if elements.shape[0] > 0:
            ...
            melt_botz = z_bot + (z_top - z_bot) * (solidus - temperature_ele_bot) / (
                temperature_ele_top - temperature_ele_bot
            )
