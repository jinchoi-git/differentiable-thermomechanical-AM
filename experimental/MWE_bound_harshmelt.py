...
learning_rate = 1e-2
work_dir = RESULTS_ROOT / f"{base_name}_bfgsbound_harshmelt"
...
def compute_losses(params):
    ...
    vm2 = jnp.clip(vm2, 0.0, jnp.inf)
    stress_loss = jnp.mean(vm2) / 50000
    ...
    margin = 100
    deficit = (jnp.maximum(tctx.liquidus + margin - Tmax_soft, 0.0) / 100) ** 4
    denom = jnp.maximum(jnp.sum(build_nodes), 1.0)
...
if __name__ == "__main__":
    ...
    if mode == "gradcheck":
        ...
        grad_check(init_params, eps=1e-5, n_checks=10)
