...
def soft_clamp(x, lo=300.0, hi=2300.0):
    # lower bound
    x1 = lo + jax.nn.softplus(x - lo)
    # upper bound
    return hi - jax.nn.softplus(hi - x1)
...
def mech(
    ...
    temperature_ip = 300 + jax.nn.softplus(temperature_ip - 300)
    ...
    K_global = assemble_global_stiffness_matrix_jax(ele_K, elements, n_dof)
    ...
    def newton_iteration(i, state):
        ...
        ridge = 1e-2    # “Tikhonov” regularization
        K_tangent = (1 - ridge) * K_tangent + ridge * ele_K

        # Assemble global tangent stiffness matrix
        K_global_tangent = assemble_global_stiffness_matrix_jax(K_tangent, elements, n_dof)
        ...
        # 4) Matrix-vector product using global tangent stiffness matrix
        def mech_matvec(x):
            # Enforce Dirichlet rows & columns:
            return Q_dof * (K_global_tangent @ (Q_dof * x))
        ...
        jax.debug.print("newton_iteration: dU_flat min={} max={}", jnp.nanmin(dU_flat), jnp.nanmax(dU_flat))
        jax.debug.print("newton_iteration: dU_flat any_nan={}", jnp.any(jnp.isnan(dU_flat)))
...
def main_function(params, smooth_weight=1e-2):
    ...
    control = jnp.concatenate([        
        soft_clamp(params[:power_on_steps], 0.75, 1.25),
    ...
    a_ = 50.0   # larger α → closer to true max
    T_max_smooth = jnp.log(jnp.sum(jnp.exp(a_*temperatures), axis=0)) / a_
    T_deficit = jax.nn.softplus(liquidus - T_max_smooth)
    ...
    jax.debug.print("main_function: loss={}", loss)
    jax.debug.print("main_function: stress_loss={}", stress_loss)
    jax.debug.print("main_function: melt_penalty={}", melt_penalty)
    jax.debug.print("main_function: smooth_penalty={}", smooth_penalty)
    jax.debug.print("main_function: loss is_nan={}", jnp.isnan(loss))
    jax.debug.print("main_function: loss is_inf={}", jnp.isinf(loss))
...
def grad_check(params, eps=1e-3, n_checks=5):
    ...
    auto_grad = np.array(jax.grad(loss_fn)(jnp.array(params)))
    print(auto_grad)

    # # Compute numeric gradient via central finite difference
    # numeric_grad = np.zeros_like(auto_grad)
