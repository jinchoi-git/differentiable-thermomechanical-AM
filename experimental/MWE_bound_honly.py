...
base_name = "4_mtml"
learning_rate = 1e-2
work_dir = RESULTS_ROOT / f"{base_name}_bfgsbound_laseronly_repeatcheck_2"
...
S_MIN, S_MAX = 0.5, 1.5  # s bounds (so h stays near baseline scale)
...
def optimize_bfgs_joint_s(
    ...
    x0 = np.asarray(
        list(np.asarray(power_blocks_init, dtype=np.float32)) + [s0], dtype=np.float64
    )
