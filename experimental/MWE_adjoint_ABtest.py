...
def _newton_core(
    ...
    maxit=50, rtol=1e-8, atol=1e-10, tikh=5e-6, debug=False,
):
    # ---- helpers ----
    ...
    def _apply_dirichlet(K_global, R, Q_dof):
        ...
        K_global = K_global + jnp.diag(1.0 - Q_dof)   # identity on fixed dofs
        R = R * Q_dof                                 # zero residual on fixed dofs
        return K_global, R

    def _compute_K_R(U_it):
        # strain, stress, consistent tangent at current iterate
        ...
        # element consistent tangent: K_tangent = ele_K + sum_q B^T[(detJ*DS) - ele_D]B
        ...
        # assemble global K, apply BCs
        K_unreg = _assemble_global(K_tangent, elem_dofs, ndof, mask_e)
        K_unreg, R = _apply_dirichlet(K_unreg, R_node, Q_dof)

        # regularized K for the **primal** Newton update only
        K_reg = K_unreg + tikh * jnp.eye(ndof)
        return K_unreg, K_reg, R, E_corr, S

    # ---- initialize residual ----
    K_unreg0, K_reg0, R0, _, _ = _compute_K_R(U_init)
    res0 = jnp.max(jnp.abs(R0))

    def cond_fun(carry):
        U_it, i, res, res_ref = carry
        thresh = jnp.maximum(atol, rtol * res_ref)
        return jnp.logical_and(i < maxit, res > thresh)

    def body_fun(carry):
        U_it, i, _res, res_ref = carry
        # compute K, R; do one Newton step with K_reg
        K_unreg, K_reg, R, _, _ = _compute_K_R(U_it)
        dU = jnp.linalg.solve(K_reg, -R).reshape(U_it.shape)
        U_new = U_it + dU

        # evaluate new residual for stopping
        Ku2, Kr2, R2, _, _ = _compute_K_R(U_new)
        res2 = jnp.max(jnp.abs(R2))
        if debug:
            jax.debug.print("Newton iter {i}: ||R||inf = {r}", i=i, r=res2)
        return (U_new, i + 1, res2, res_ref)

    U_star, iters, res, res_ref = jax.lax.while_loop(
        cond_fun, body_fun, (U_init, 0, res0, res0)
    )

    # final K/R at converged U*
    K_unreg_star, K_reg_star, R_star, E_corr_star, S_star = _compute_K_R(U_star)
    if debug:
        jax.debug.print("Newton done: iters={it}, ||R||inf={r}", it=iters, r=res)

    # return both Ks so: primal uses K_reg, adjoint uses K_unreg
...
# ------------- A/B hooks -------------
def newton_solve_through_fixed(
    U_init, temperature_ip, E_th,
    elements, nodes,
    ele_K, ele_B, B_T, ele_D, ele_detJac,
    shear, bulk, a, Y,
    ...
    # Reuse the same builder you already have (but DO NOT use while_loop here)
    def _compute_K_R(U_it):
        E_base = jax.vmap(compute_E, in_axes=(0, 0, None))(elements, ele_B, U_it)
        E_corr = (E_base - E_th) * mask_e[:, None, None]
        S, DS, _, _, _ = constitutive_problem(E_corr, Ep_prev, Hard_prev, shear, bulk, a, Y)
        ...
        # Assemble & apply BCs
        K_unreg = jnp.zeros((ndof, ndof))
        def add_elem(K, e_idx):
            Ke = K_tangent[e_idx] * mask_e[e_idx]
            dofs = elem_dofs[e_idx]
            return K.at[dofs[:, None], dofs[None, :]].add(Ke), None
        K_unreg, _ = jax.lax.scan(add_elem, K_unreg, jnp.arange(K_tangent.shape[0]))
        # Dirichlet rows/cols + masked residual
        K_unreg = K_unreg * (Q_dof[:, None] * Q_dof[None, :]) + jnp.diag(1.0 - Q_dof)
        R       = R_node * Q_dof
        # Regularize only for the primal step
        K_reg   = K_unreg + tikh * jnp.eye(ndof)
        return K_unreg, K_reg, R

    def body(i, carry):
        U_it = carry
        _, K_reg, R = _compute_K_R(U_it)
        dU = jnp.linalg.solve(K_reg, -R).reshape(U_it.shape)
        return U_it + dU

    # IMPORTANT: pass a Python int for maxit so bounds are static
    U_star = jax.lax.fori_loop(0, int(maxit), body, U_init)
    return U_star
...
def newton_solve_implicit_bwd(saved, bar_Ustar):
    # (U_star, K_star, _R_star, _E_corr_star, _S_star,
    #  temperature_ip, E_th, elements, nodes,
    #  ele_K, ele_B, B_T, ele_D, ele_detJac,
    #  shear, bulk, a, Y,
    ...
    # jax.debug.print("any NaN in K*: {}", jnp.isnan(K_star).any())
    # jax.debug.print("any NaN in lam: {}", jnp.isnan(lam).any())
    ...
    def R_masked(temperature_ip_, E_th_, Ep_prev_, Hard_prev_):
        ...
        R_node = jnp.zeros((ndof,))
        R_node = R_node.at[elem_dofs.flatten()].add(F_e.flatten())
        return R_node * Q_dof  # <- mask residual like the primal system

    vjp_fn = jax.vjp(R_masked, temperature_ip, E_th, Ep_prev, Hard_prev)[1]
    g_temp_ip, g_Eth, g_Ep, g_Hard = vjp_fn(lam)

    # # Optional guardrails while bringing this up (remove once clean)
    # g_temp_ip = jnp.nan_to_num(g_temp_ip, nan=0.0, posinf=0.0, neginf=0.0)
    # g_Eth     = jnp.nan_to_num(g_Eth,     nan=0.0, posinf=0.0, neginf=0.0)
    # g_Ep      = jnp.nan_to_num(g_Ep,      nan=0.0, posinf=0.0, neginf=0.0)
    # g_Hard    = jnp.nan_to_num(g_Hard,    nan=0.0, posinf=0.0, neginf=0.0)
...
def mech(
    ...
    USE_IMPLICIT = False  # flip to False for the A/B check
    ...
    solve = newton_solve_implicit if USE_IMPLICIT else newton_solve_through_fixed
    U_it  = solve(
        U_guess, temperature_ip, E_th,
        elements, nodes,
        ele_K, ele_B, B_T, ele_D, ele_detJac,
        shear, bulk, a, Y,
        Ep_prev, Hard_prev,
        mask_e, Q_dof, elem_dofs,
        maxit=3,   # fixed iteration count
        # rtol/atol are only for the implicit (while_loop) variant
    )
    dU = U_it - U
...
def grad_check(params, eps=1e-3, n_checks=5):
    ...
    for i in range(n_checks):
        p_plus  = params.copy();  p_plus[i]  += eps * (1 + np.abs(params[i]))
        p_minus = params.copy();  p_minus[i] -= eps * (1 + np.abs(params[i]))
...
def dirderiv_check(loss_fn, p, g, v=None, eps_list=(1e-6,3e-6,1e-5,3e-5,1e-4,3e-4,1e-3)):
    p = jnp.array(p); g = jnp.array(g)
    if v is None:
        key = jax.random.PRNGKey(0)
        v = jax.random.normal(key, p.shape)
    v = v / (jnp.linalg.norm(v) + 1e-12)

    gt_v = float(jnp.dot(g, v))
    print(f"g·v = {gt_v: .6e}")
    for eps in eps_list:
        fd = float(loss_fn(p + eps*v) - loss_fn(p - eps*v)) / (2*eps)
        rel = abs(fd - gt_v) / (1 + max(abs(fd), abs(gt_v)))
        print(f"eps={eps: .1e}  FD_dir={fd: .6e}  rel.err={rel: .3e}")
...
if __name__ == "__main__":   
    ...
    if mode == "gradcheck":
        ...
        # grad_check(init_params, eps=1e-4, n_checks=3)

        def loss_fn(p): return main_function(jnp.array(p))[0]
        p0 = jnp.array(init_params)
        g0 = jax.grad(loss_fn)(p0)
        dirderiv_check(loss_fn, p0, g0)
