...
base_name = "4_mtml"
learning_rate = 1e-2
work_dir = RESULTS_ROOT / f"{base_name}_bfgs"
...
conds = jnp.ones((n_e, 8)) * cond_val
stefan_boltz = (
    5.670374419e-8  # W*m^-2*K^-4 (keep units consistent with your nondim if any)
)
...
def expand_params_to_controls(
    params, power_on_steps, power_off_steps, min_power=0.5, max_power=1.5
):
    """
    ...
    knots = jnp.arange(
        0, power_on_steps, stride, dtype=jnp.float32
    )  # [0,10,20,...,490] length 50
...
#     # ------------------ Total loss ------------------
#     total_loss = STRESS_W * stress_loss

#     jax.debug.print(
#         "loss: total={tot:.4e} | stress={sl:.4e} ",
#         tot=total_loss, sl=stress_loss,
#     )
...
def optimize_bfgs(params_init, num_iterations, run_dir, learning_rate=None):
    ...
    res = spo.minimize(
        ...
        jac=True,
        options=dict(
            maxiter=int(num_iterations), gtol=1e-6, ftol=1e-10, maxcor=10, maxls=40
        ),
        callback=cb,
    )
