...
learning_rate = 3e-2
work_dir = RESULTS_ROOT / "code_refactor"
...
a1 = 10000
young1 = jnp.asarray(
    np.loadtxt("./materials/TI64_Young_Debroy.txt")[:, 1] / 1e6, dtype=DTYPE
)
temp_young1 = jnp.asarray(
    np.loadtxt("./materials/TI64_Young_Debroy.txt")[:, 0], dtype=DTYPE
)
Y1 = jnp.asarray(
    np.loadtxt("./materials/TI64_Yield_Debroy.txt")[:, 1] / 1e6 * np.sqrt(2 / 3),
    dtype=DTYPE,
)
temp_Y1 = jnp.asarray(
    np.loadtxt("./materials/TI64_Yield_Debroy.txt")[:, 0], dtype=DTYPE
)
scl1 = jnp.asarray(np.loadtxt("./materials/TI64_Alpha_Debroy.txt")[:, 1], dtype=DTYPE)
temp_scl1 = jnp.asarray(
    np.loadtxt("./materials/TI64_Alpha_Debroy.txt")[:, 0], dtype=DTYPE
)
...
def expand_params_to_controls(
    params, power_on_steps, power_off_steps, n_blocks=N_BLOCKS
):
...
def main_function(params):
    ...
    jax.debug.print(
        "loss (mean VM^2): {meanv:.4e}, max VM^2: {maxv:.4e}",
        meanv=stress_loss,
        maxv=jnp.max(vm2),
    )
...
def optimize_bfgs(params_init, num_iterations, run_dir, learning_rate=None):
    ...
    def fun_and_grad(x_np64):
        ...
        control, _ = expand_params_to_controls(
            jnp.asarray(x_np64, dtype=DTYPE), power_on_steps, power_off_steps
        )
    ...
    res = spo.minimize(
        ...
        jac=True,
        options=dict(
            maxiter=int(num_iterations), gtol=1e-8, ftol=1e-12, maxcor=10, maxls=40
        ),
        callback=cb,
    )
